...
//	$_SESSION['user_id'] = $dbHandler->getUserId($username);
	...
	public function registerUser(){
		...
		if (empty($username) || empty($password) || empty($passwordRepeat) || empty($email)){
			...
			return array('msg' => 'Fill in all the fields');

		}

		if (!empty($this->dbHandler->getUsername($username))){
			// account with given username exists
			return array('msg' => 'Username already in use');
		}

		else if (!filter_var($email, FILTER_VALIDATE_EMAIL)){
			// invalid email adress
			return array('msg' => 'Invalid e-mail adress');

		}

		else if (!empty($this->dbHandler->getEmail($email))){
			// account with given email exists
			return array('msg' => 'E-mail already in use');
		}

		else if (!preg_match("/.{8,}/", $password)){
			// password must be at least 8 characters long
			return array('msg' => 'Password must contain more than 8 characters');
		}

		else if (!preg_match("/\d/", $password)){
			// password must contain at least one number
			return array('msg' => 'Password must contain numbers');
		}

		else if (!preg_match("/[a-z]/", $password)){
			// password must contain lower case
			return array('msg' => 'Password must contain lower case letters');
		}

		else if (!preg_match("/[A-Z]/", $password)){
			// password must contain upper case letters
			return array('msg' => 'Password must contain upper case letters');
		}

		else if ($password !== $passwordRepeat){
			// passwords dont match
			return array('msg' => "Password don\'t match");
		}
